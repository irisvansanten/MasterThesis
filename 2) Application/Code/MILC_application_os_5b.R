...
output_MILC = function(theta_LC, var_LC, theta, data_type, approach, nboot, ncat){
  ...
  write.csv2(p_mean, file=sprintf("mean_%s_approach%s_os_test.csv", data_type, approach), quote=FALSE, row.names=F)
  # 2) Bias estimation
  ...
  write.csv2(p_bias, file=sprintf("bias_%s_approach%s_os_test.csv", data_type, approach), quote=FALSE, row.names=F)
  # 3) Variance
  ...
  # 5) RMSE
  p_RMSE = sqrt(p_MSE)
  write.csv2(p_RMSE, file=sprintf("RMSE_%s_approach%s_os_test.csv", data_type, approach), quote=FALSE, row.names=F)
  ...
  plot.data = data.frame(category = rep(1:ncat),
                         ...
                         RMSE = p_RMSE,
                         sd = sqrt(p_VAR)*1.96)
  write.csv2(plot.data, file=sprintf("plot_table_%s_approach%s_os_test.csv", data_type, approach), quote=FALSE, row.names=F)
}
...
plot_MILC = function(pms, data_type, approach, theta, ncat){
  ...
  for(pm in 1:length(pms)){
    ...
    table_pm = read.csv(file=sprintf("plot_table_%s_approach%s_os_test.csv", data_type, approach ), header=T, sep=";")
    ...
    if(pm == 1){
      ...
      ggsave(sprintf("plot_%s_%s_approach%s_os_test.pdf", pms[pm], data_type, approach),width = 5, height = 5)
    }else{
...
for(dat in data_sets){
  ...
  write.csv2(perc_mv, file=sprintf("perc_mv_%s_os_test.csv", dat), quote=FALSE, row.names=F)
  n_data = nrow(data_set)
  write.csv2(n_data, file=sprintf("n_data_%s_os_test.csv", dat), quote=FALSE, row.names=F)
  ...
  write.csv2(theta, file=sprintf("theta_%s_os_test.csv", dat), quote=FALSE, row.names=F)
  for(approach in 1:napproaches){
    ...
    write.csv2(n, file=sprintf("n_approach%s_%s_os_test.csv", approach, dat), quote=FALSE, row.names=F)
